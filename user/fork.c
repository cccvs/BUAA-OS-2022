...
pgfault(u_int va)
{
	...
	if(!(perm & PTE_COW)){
		user_panic("err in fork.c:pgfault():\n");
		return;
	}
	...
	if(syscall_mem_map(0, tmp, 0, round_va, PTE_V | PTE_R) < 0)
		user_panic("err in fork.c:pgfault():\n");
...
fork(void)
{
	...
	for(i = 0; i < USTACKTOP; i += PDMAP){
		if((*vpd)[PDX(i)]){
			for(j = 0; i + j < USTACKTOP && j < PDMAP; j += BY2PG){
				if((*vpt)[VPN(i + j)])
	...
	if(syscall_mem_alloc(newenvid, UXSTACKTOP - BY2PG, PTE_V | PTE_R) < 0)
		user_panic("fork failed in syscall_mem_alloc");
	if(syscall_set_pgfault_handler(newenvid, __asm_pgfault_handler, UXSTACKTOP) < 0)
		user_panic("fork failed in syscall_set_pgfault_handler");
	if(syscall_set_env_status(newenvid, ENV_RUNNABLE) < 0)
		user_panic("fork failed in syscall_set_env_status");
	return newenvid;
}
